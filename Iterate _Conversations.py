...
# Setup logging
setup_logging('iterate_conversations.log')
    ...
    for row in range(2, post_chat_data_ws.max_row + 1):
        if not post_chat_data_ws.cell(row=row, column=16).value:  # Column P is the 16th column
            try:
                ...
                logging.error(f"Encountered an error at row {row}: {e}")
            ...
            logging.info(f'Completed row {row} at {current_timestamp}. Remaining iterations: {post_chat_data_ws["R1"].value}')
